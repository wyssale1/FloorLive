...
export default function GamesPage() {
  ...
  const GameSection = ({ title, games, icon }: { title: string; games: typeof allGames; icon?: React.ReactNode }) => {
    ...
    const getSectionBg = () => {
      if (title === "Live Games") return "bg-primary/5"
      if (title === "Today's Games") return "bg-muted/30"
      if (title === "Recent Results") return "bg-muted/20"
      return "bg-muted/10"
    }
    ...
    return (
      <section className="space-y-3">
        ...
        <div className={`${getSectionBg()} rounded-lg overflow-hidden`}>
          {games.map((game, index) => (
            <div key={game.id} className="p-3 hover:bg-background/50 transition-colors">
              <GameCard game={game} />
            </div>
          ))}
        </div>
